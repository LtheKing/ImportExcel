...
class NasabahModel extends CI_Model {
	...
	public function view_search_nomor($IDBank,$search_nomor){
		$this->load->library('pagination'); // Load librari paginationnya
		$query = "SELECT * FROM datanasabah where IDBank='$IDBank' 
		and (NomorKartu like '%$search_nomor%' 
		or NamaNasabah like '%$search_nomor%')
		order by ID DESC "; // Query untuk menampilkan semua data siswa
		
		$config['base_url'] = base_url('index.php/nasabah/search_nomor/'.$IDBank.'');
		$config['total_rows'] = $this->db->query($query)->num_rows();
		$config['per_page'] = 10;
		$config['uri_segment'] = 4;
		$config['num_links'] = 3;
	...
	public function view_search_kategori($IDBank,$search_kategori,$search_tipe){
		$this->load->library('pagination'); // Load librari paginationnya

		if($search_tipe==1){
			$query = "SELECT * FROM datanasabah where IDBank='$IDBank' 
		and Action like '%$search_kategori%'
		order by ID DESC "; // Query untuk menampilkan semua data siswa	
		}
		else{
			$query = "SELECT * FROM datanasabah where IDBank='$IDBank' 
		and Status like '%$search_kategori%'
		order by ID DESC "; // Query untuk menampilkan semua data siswa
		}
		
		
		$config['base_url'] = base_url('index.php/nasabah/search_kategori/'.$IDBank.'');
		$config['total_rows'] = $this->db->query($query)->num_rows();
		$config['per_page'] = 10;
		$config['uri_segment'] = 4;
		$config['num_links'] = 3;
